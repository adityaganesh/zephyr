...
static int icm20948_get_device_id(const struct device *dev, uint8_t *device_id)
{
    const struct icm20948_config *cfg = dev->config;
...
static int icm20948_init(const struct device *dev)
{
    ...
    int err = icm20948_get_device_id(dev,&id);

    if(id == ICM20948_DEVICE_ID){
        LOG_INF("ICM20948 detected.");
    } else {
        LOG_ERR("Device ID mismatch. Expected 0x%X, got 0x%X", ICM20948_DEVICE_ID, id);
        return -EIO;
    }

	err = icm20948_reset(dev);
